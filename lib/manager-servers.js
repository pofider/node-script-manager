    ...
    self.workersCluster.on('message', function (rawM) {
      ...
      if (m.action === 'register') {
        ...
        reqOptions.timeoutRef = setTimeout(function () {
          ...
          self._runningRequests = _.without(self._runningRequests, _.findWhere(self._runningRequests, {
            rid: reqOptions.rid
          }))
...
ScriptsManager.prototype.execute = function (inputs, options, cb) {
  ...
  request({
    ...
    body: messageHandler.serialize(body)
  }, function (err, httpResponse, rawBody) {
    ...
    var body = messageHandler.parse(rawBody)
