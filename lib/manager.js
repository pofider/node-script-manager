...
function findFreePort(cb) {
    ...
    require('portscanner').findAPortNotInUse(15000, 35530, process.env.OPENSHIFT_NODEJS_IP || '127.0.0.1', function(error, port) {
...
ScriptsManager.prototype.execute = function (inputs, options, cb) {
    ...
    var servHost = process.env.OPENSHIFT_NODEJS_IP||"localhost";
    request({
        method: "POST",
        url: "http://" + servHost + ":" + this.options.port,
        body: body,
        json: true
