...
if (!cluster.isMaster) {
  var startListening = function (port, host) {
    var server = require('http').createServer(function (req, res) {
      processRequest(req, res, handleErrorInWorker)
    })

    function handleErrorInWorker (err, res) {
      try {
...
function processRequest (req, res, handleErrorInWorker) {
  var body = ''

  req.on('error', function (err) {
    handleErrorInWorker(err, res)
  })
